/*
This is the source for QUIVER, Qualitative Inference from Variance in Execution Rate.  It is based on the HOMB benchmark, but where the HOMB benchmark strived to be as
fast and efficient as possible, QUIVER focuses on diversity of tests and functionality, particularly at low level system performance.  
...
/* Finds number of tasks and threads, gets the calling task's number, and sets distributed grid size */
...
/* Does all the actual solving: comunicationa and computation.  (Includes parallel region) */
...
void statistics(double **times, double *minTime, double *meanTime, double *maxTime, double *stdvTime, double *NstdvTime);

/* File Output */
void fileIO(double **times, double minTime, double meanTime, double maxTIme, double NstdvTime);

/* Standard Ouput */
void stdoutIO(double **times, double minTime, double meanTime, double maxTime, double NstdvTime);

/* Inverse of the erf function on a random number between -1 and 1 to given resolution (bisection method) */
float ierf(int res);

/* Prints either the new or old plane of the grid matrix for diagnostics */
void printMatrix(float ***t, int plane);

/* Global Context (All variables in Global Context are set once and then constant over all threads and iterations) */
int NR, NC, NITER, EXTRA; // Number of Rows, Number of Columns, Number of Iterations (samples), extra FLOP
int vOut, sOut, fOut, tOut, pHeader, pContext; // Input/Ouput logicals
char *fileOutName, *timesOutName; // Output file names 
int waiter, duration; float delay; // Delay location, duration(iterations), and time(s)
int nPEs, nThreads, nrl; // Number of Tasks and Threads, number of rows in local section of distributed grid
int disorder, shuffle, barrier, reduce; //Logicals for tricking prefetcher and shuffling PE numbers and per iteration MPI calls
...
int main(int argc, char *argv[]) {
  ...
  /* Timing measurements */
  double startTime, endTime, meanTime = 0., minTime = HUGE, maxTime = 0., stdvTime = 0., NstdvTime = 0.;
  ...
  if (myPE == ROOT) {
    statistics(times, &minTime, &meanTime, &maxTime, &stdvTime, &NstdvTime);
  }
  ...
  if (myPE == ROOT) {
    ...
    if ( (fOut + tOut) > 0 )
      fileIO(times, minTime, meanTime, maxTime, NstdvTime);
    /* Check for Standard Output */
    if ( (sOut + vOut) > 0 )
      stdoutIO(times, minTime, meanTime, maxTime, NstdvTime);
  }
...
void initContext(int argc, char *argv[]){
  ...
  /* Default with order */
  disorder = 0; shuffle = 0; barrier = 0; reduce = 1;
  ...
  for (int i = 1; i < argc; i++){  
    ...
    if ( strcmp("--help",argv[i]) == 0 ){
      printf("Usage: ./homb {-NR num-rows -NC num-cols} | {-NRC num-rows num-calls} -NITER num-iter [-EXTRA extra-flop] [-delay rank time(s) duration(iterations)] \n \
             [-v | -s] [-vf verbose-out-filename] [-tf time-out-filename] [-help] \n");
      exit(EXIT_SUCCESS);
    }
...
void printContext(void){
  ...
  if (barrier != 0)
    printf("Barrier was added to the end of each loop \n");
  if (reduce != 1)
    printf("Reduction removed from the end of each loop \n");
  if (vOut)
